...
const main = () => {
    const obj = {
        main: {
            Background: (p: unknown) => (
                <div
                    id={style.playlistBg}
                    style={{ backgroundImage: `url(${p.data.images[0].url})` }}
                >
                    <div id={style.playlistBgGradient}>{p.children}</div>
                </div>
            ),
            IndividualPlaylist: (p: unknown) => (
                <obj.main.Background data={p.data}>
                    <div id={style.playlistSpace}>
                        ...
                        <div id={style.playlistDataSpace}>
                            <div id={style.playlistName}>{p.data.name}</div>
            ...
            SpotifyMainPage: (p: unknown) => (
                <obj.main.Background data={p.bgData}>
                    <div id={style.playlistsSpace}>{p.children}</div>
                </obj.main.Background>
            ),
            pages: {
                AllPlaylists: (p: unknown) => (
                    <div id={style.allPlaylists}>{p.children}</div>
                ),
            },
        ...
        nav: {
            ...
            SpotifyMainPageButton: (props: unknown) => (
                <Link
            ...
            navMenu: class navMenu extends React.Component<Props, UnknownObj> {
                id: string;
                style: unknown;
                className: unknown;
                ...
                render() {
                    return (
                        <div
                            id={this.id}
                            className={style.navFloatingMenu}
                            style={this.style}
                        ></div>
                    );
                }
